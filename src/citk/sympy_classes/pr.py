...
class Pr(Symbol):
    ...
    def apply_bayes(self, flip:Set[Variable]=None) -> sp.Expr:
        """
        Apply Bayes' theorem to the probabilistic expression to return P(A,B)/P(B) from P(A|B).
    ...
    def apply_bayes_inverse(expr:sp.Expr) -> sp.Expr:
        ...
        """
        Apply Bayes' theorem to the probabilistic expression to return Pr(A|B) from P(A,B)/P(B)
